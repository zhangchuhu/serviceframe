...
namespace server
{
	namespace grp
	{
		...
		public:
			...
			void GBroadcastDouble2(uint32_t uSenderId, uint32_t uHashKey, uint32_t uBroadcastId, protocol::gtransfer::EOfflineMsgMode emMode,
				uint32_t Uri, const sox::Marshallable &obj, bool bReliable = false)
			{
				server::gtransfer::PSS_GBroadcast2 objBroadcast(Uri, obj);
				objBroadcast.m_stGBoardcast.SetSendId(uSenderId);
				...
				objBroadcast.m_stGBoardcast.SetGid(uBroadcastId);
				objBroadcast.m_stGBoardcast.SetKey(uHashKey);
				objBroadcast.m_stGBoardcast.m_bSync = true;
				objBroadcast.m_stGBoardcast.m_vecSyncUids.clear();

				log(Info, "[%s] (HashKey=%d, uBroadcastGid=%d)", __FUNCTION__, uHashKey, uBroadcastId);

				bReliable?GHashDispatchDouble(uHashKey, SUFFIX_GTRANS, objBroadcast.uri, objBroadcast, true):GHashDispatchDouble(uHashKey, SUFFIX_GTRANS, objBroadcast.uri, objBroadcast);
			}
